...
The Capacitor build is the native entry point for the Shell. Native builds render only the dedicated `src/app/mobile/` package: a phone-focused TRS sign-in view and a secure member home. The hosted browser build keeps the existing Shell header and `/myhealth` Health MFE route; it never renders the mobile views. Native mode requires the Shell's own `capacitor://localhost` (or native localhost) origin, so loading a hosted Shell route inside the legacy Pension WebView remains a web passthrough instead of switching to the dedicated mobile UI.
